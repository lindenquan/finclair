<script lang="ts">
import { authStore } from "$lib/stores/auth.svelte";
</script>

<main class="flex min-h-dvh items-center justify-center p-4">
	<div class="card w-full max-w-sm bg-base-200 shadow-xl">
		<div class="card-body items-center text-center">
			<div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-base-300 text-3xl font-bold text-primary" aria-hidden="true">
				F
			</div>
			<h1 class="card-title text-2xl font-extrabold">Finclair</h1>
			<p class="text-sm text-base-content/60">Your personal financial clarity.</p>
			<p class="mt-2 text-xs text-base-content/40">
				Sign in with Google to sync your financial data securely to your Google Drive.
			</p>
			<div class="card-actions mt-6 w-full">
				<button
					class="btn btn-primary w-full gap-2"
					onclick={() => authStore.login()}
					disabled={authStore.loading}
				>
					{#if authStore.loading}
						<span class="loading loading-spinner loading-sm"></span>
						Signing in…
					{:else}
						<svg class="h-5 w-5" viewBox="0 0 24 24" aria-hidden="true">
							<path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" />
							<path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
							<path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
							<path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
						</svg>
						Sign in with Google
					{/if}
				</button>
			</div>
		</div>
	</div>
</main>
