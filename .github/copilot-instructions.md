...
- **Mobile OS** — Android, iOS (iPhone/iPad)

The app is an SPA/PWA running in a browser engine on all platforms. Use only Web APIs that are supported across these targets.

## Code Style

- **Indentation: 2 spaces — never tabs.** This applies to all files: `.ts`, `.svelte`, `.js`, `.json`. Biome enforces this and will fail CI if tabs are used.
...
Skills are specialized workflows for specific tasks. **When a skill applies, you MUST read its SKILL.md file as your very first action before doing anything else.**

| Skill | Apply when | File |
|---|---|---|
| Code Review | Asked to review code, audit changes, check quality, find bugs | [`.github/skills/code-review/SKILL.md`](.github/skills/code-review/SKILL.md) |
