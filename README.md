...
Google OAuth via Firebase Auth using `signInWithPopup`. Login is a client-side gate — unauthenticated users see a sign-in page, authenticated users see the app. The popup captures the OAuth access token needed for Google Drive API. Users can also choose offline mode (no login) with local-only data.
